...
The second reason is more simple: we already had the Webpack v4 upgrade on our roadmap. Being a major version bump, it was possible that any optmizations we implemented while on Webpack v3 would be moot once the (eventual) upgrade was completed. Therefore, we agreed it was worth it to upgrade now, see what it gives us, and improve things from there.

(Watch use of 'FEI' acronym, depending on where your intro to the team ends up.)
...
3. Upgrade/reconfigure as necessary to resolve each error.
...
If you don't see any mentions of Webpack in the release history, check the Github issues for the package. There are still several loaders/plugins that have not yet upgraded to be compatible with Webpack v4, and these will usually have one or more Github issues related to this incompatibility.
...
(I really like this teaser. Do you have a working title? Are you thinking "Part 1"?)
